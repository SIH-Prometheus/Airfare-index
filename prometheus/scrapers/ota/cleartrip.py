...
class CleartripSpider(scrapy.Spider):
    """Spider to scrape flight quotes from Cleartrip (OTA) using Playwright."""
        ...
        **kwargs,
        ...
        if "/" in raw_date:
            ...
            if len(parts) == 3:
                ...
                self.date_formatted = f"{int(d):02d}/{int(m):02d}/{y}"
            else:
                self.date_formatted = raw_date
        elif "-" in raw_date:
            parts = raw_date.split("-")
            if len(parts) == 3:
            ...
            self.date_formatted = f"{d}/{m}/{y}"
        else:
            self.date_formatted = raw_date

        self.headless = str(headless).lower() in ("true", "1", "yes")

        # Cleartrip one-way search URL
        self.start_url = (
            f"https://www.cleartrip.com/flights/results?"
            f"from={self.origin}&to={self.destination}&depart_date={self.date_formatted}"
    ...
    def start_requests(self):
        logger.info(
            f"Searching Cleartrip: {self.origin} -> {self.destination} on {self.date_formatted} (Lead time: {self.lead_time})"
        )
    ...
    async def parse(self, response):
        ...
        try:
            logger.info("Cleartrip page loaded. Waiting for flight results...")

            # Wait for search results container
            try:
                await page.wait_for_selector("div.br-12, div[class*='ba-solid'], button:has-text('Book')", timeout=25000)
                logger.info("Cleartrip flight cards detected.")
            except Exception:
                logger.warning("Timeout waiting for Cleartrip flight card elements.")
            ...
            logger.info(f"Extracted {flight_count} flights from Cleartrip.")

        except Exception as exc:
            logger.error(f"Cleartrip parse error: {exc}", exc_info=True)
        finally:
            await page.close()

    def parse_flight_cards(self, selector):
        """Extract FlightItems from rendered Cleartrip HTML selector."""
        now_str = datetime.now(timezone.utc).isoformat()
        cards = selector.css("div.br-12, div[class*='ba-solid'][class*='br-12']")
